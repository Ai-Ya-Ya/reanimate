...
genGolden path = do
  ...
  -- DEBUG: Save the actual output for inspection
  -- This answers "How can I save said dumped SVGs?"
  let debugDir = "dist" </> "debug"
  createDirectoryIfMissing True debugDir
  let dumpPath = debugDir </> takeFileName path <.> "actual.svg"
  BS.writeFile dumpPath out
